...
make_fasteignaskattur <- function() {
  d <- here("greinar", "fasteignagjold", "data", "fasteignaskattur_clean.csv") |> 
    read_csv() |> 
    mutate(
      sveitarfelag = fct_reorder(sveitarfelag, fasteignaskattur, .fun = max),
      svf_ordered1 = fct_reorder(sveitarfelag, fasteignaskattur, .fun = \(x) head(x, 1)),
      svf_ordered2 = fct_reorder(sveitarfelag, fasteignaskattur, .fun = \(x) tail(x, 1))
    ) 
  ...
  p1 <- d |> 
    ...
    ggplot(aes(fasteignaskattur, svf_ordered1)) +
  ...
  p2 <- d |> 
    ...
    ggplot(aes(fasteignaskattur, svf_ordered2)) +
    geom_text_interactive(
      aes(x = 0, label = str_c(sveitarfelag, " "), data_id = sveitarfelag),
      hjust = 1,
      size = text_size
  ...
  p3 <- d |> 
    ggplot(aes(year, fasteignaskattur))
  ...
  p <- (p1 + p2) / p3 +
    plot_annotation(
      title = "Fasteignaskattur eftir sveitarfélagi",
