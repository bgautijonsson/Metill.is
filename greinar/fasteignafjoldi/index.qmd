...
if (!file.exists("data/data.parquet")) {
  ...
  mannfjoldi <- pxweb_get(
    ...
    filter(sveitarfelag != "Alls",
           aldur != "Alls") |> 
    mutate(
      aldur = ifelse(aldur == "Á 1. ári", 0, aldur),
      aldur = parse_number(aldur)
    ) |> 
    select(-kyn) |> 
    mutate(
...
p <- plot_dat |> 
  ...
  annotate(
    ...
    geom = "text",
    label = str_c(
      "Seltjarnarnesbær er eina sveitarfélagið þar sem",
      "\nfasteignum fjölgaði hraðar en fullorðnum, en",
      "\nfullorðnum fækkaði um 67 frá 2020 til 2023."
    ),
    vjust = 1,
    hjust = 0,
    color = "grey40",
