...
(
  plot_dat |> 
    group_by(dags) |> 
    mutate(hlutf = n / sum(n)) |> 
    ungroup() |> 
    crossing(plot = 1:3) |> 
    mutate(
      keep = case_when(
        (plot == 1) & (tegund == "Annad") ~ TRUE,
        (plot == 3) & (tegund == "Opinbert") ~ TRUE,
        (plot == 2) ~ TRUE
      ),
      colour = str_c(tegund, plot),
      plot = c(
    ...
    mutate(
      text = str_c(
        "<b>", tegund, " vinnumarkaður</b>", "<br>",
        "Dagsetning: ", dags, "\n",
        "Fjöldi: ", number(n, decimal.mark = ",", big.mark = "."), "\n",
        "Hlutfall: ", percent(hlutf, accuracy = 0.1, decimal.mark = ",", big.mark = ".")
      )
    ) |> 
    ggplot(aes(dags, n, text = text)) +
    geom_line(aes(col = colour, group = colour)) +
    scale_x_date(
      date_breaks = "2 year",
      date_labels = "%Y"
    ) +
