...
plot_dat <- d |> 
  mutate(
    bygg_per_pers = fjoldi / mannfjoldi * 1e3,
    fullordin_per_bygg = mannfjoldi_fullordin / cum_fjoldi
  ) |> 
  select(ar, sveitarfelag, bygg_per_pers, fullordin_per_bygg) |> 
  mutate(
...
p1 <- pd |>
  ggplot(aes(fullordin_per_bygg, sveitarfelag)) +
  geom_point(
    size = 2
  ) +
  geom_segment(
