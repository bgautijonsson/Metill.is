...
m1 <- mapview(
  plot_dat,
  zcol = "value",
  layer.name = "Nýjar fasteignir<br>á 1.000 fullorðna<br>(2018 - 2023)",
...
plot_dat <- svf_shp |> 
  ...
  mutate(
    group = ifelse(str_detect(sveitarfel, "Reykjav"), "RVK", "Nágrannar")
  ) |> 
  group_by(group) |> 
  summarise(
    value = sum(value)
...
m2 <- mapview(
  ...
  at = plot_dat$value,
  layer.name = "Nýjar fasteignir<br>(2018-2023)",
  popup = popupGraph(p, width = width, height = 0.621 * width)
)
```
